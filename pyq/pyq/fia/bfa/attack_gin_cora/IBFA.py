...
class InjectivityBFA(BFABase):
    ...
    def run_attack(self, model, data=None, mask1=None, mask2=None, batch_sz=None, max_flips=15, fold=0):
        data = data
        ...
        self.loss = self.criterion(output[mask1], output[mask2])
        ...
        for a in range(max_flips-self.bit_counter):
            ...
            for name, module in model.named_modules():
                if isinstance(module, (TransformationLayerQuantizerWrapper, GenericLayerQuantizerWrapper)):
                    ...
                    self.loss_dict[name] = self.criterion(output1[mask1],
                                                          output1[mask2]).item()
