        ...
        for i, m in enumerate(model.modules()):
            if isinstance(m, (TransformationLayerQuantizerWrapper, GenericLayerQuantizerWrapper)):
                if not hasattr(m, 'b_w'):
                    m.N_bits = 2  # 8 # is this correct? #is_quantized --> layer has the values (not anymore?)
        ...
        for name, m in model.named_modules():
            if name == chosen_module:
                ...
                tmp_list = ["module_idx",  # module index in the net
                            ...
                            [weight_idx//m._wrapped_object.weight.data.shape[1], weight_idx%m._wrapped_object.weight.data.shape[1]],  # attacked weight index in weight tensor
