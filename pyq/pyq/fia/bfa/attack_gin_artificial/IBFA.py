from pyq.fia.bfa.attack_gin_artificial.BFAbase import BFABase
...
class InjectivityBFA(BFABase):
    ...
    def run_attack(self, model, data=None, target=None):
        ...
        out1, out2 = model(data1), model(data2)
        ...
        for _ in range(101):
            #print(self.loss_min, self.loss.item(), flush=True)
            self.n_bits2flip += 1
            ...
            for name, module in model.named_modules():
                if isinstance(module, (TransformationLayerQuantizerWrapper, GenericLayerQuantizerWrapper)):
                    if module._wrapped_object.weight.grad is None:
                        continue
            ...
            if self.loss_min < self.loss.item() or self.loss_min == 0:
                break
        if i == 100:
            return None
            ...
            if name == max_loss_module:
                ...
                for i in range(attack_weight_idx.size()[0]):
                    ...
                    tmp_list = [module_idx,  # module index in the net
                                ...
                                weight_post,
                                self.loss_min# weight magnitude after attack
                                ]
                    attack_log.append(tmp_list)
                    if self.bit_counter + (i + 1) >= 5: break
