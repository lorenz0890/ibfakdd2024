...
class ProgressiveBFA(BFABase):
    ...
    def run_attack(self, model, data=None, target1=None, target2=None, batch_sz=None, max_flips=15, fold=0):
        ...
        self.loss = self.criterion(output[data.train_mask[:,fold]][target1:target1 + batch_sz], pred[data.train_mask[:, fold]][target1:target1 + batch_sz])
        ...
        for a in range(max_flips-self.bit_counter):
            print(a, self.loss_max, self.loss_init, self.loss.item(), flush=True)
            self.n_bits2flip += 1
            ...
            for name, module in model.named_modules():
                if isinstance(module, (TransformationLayerQuantizerWrapper, GenericLayerQuantizerWrapper)):
                    ...
                    output = model(data.x, data.edge_index, data.edge_weight)  # [data.train_mask]
                    quantize(model)

                    self.loss_dict[name] = self.criterion(output[data.train_mask[:, fold]][target1:target1 + batch_sz],
                                                          data.y[data.train_mask[:, fold]][target1:target1 + batch_sz]).item()
