...
class ProgressiveBFA(BFABase):
    ...
    def run_attack(self, model, data=None, target=None, max_flips=5):
        ...
        output = model(x, data.edge_index, data.batch)
        ...
        self.loss = self.criterion(output[is_labeled], target[is_labeled])
        ...
        for i in range(10):
            ...
            for name, module in model.named_modules():
                if isinstance(module, (TransformationLayerQuantizerWrapper, GenericLayerQuantizerWrapper)):
                    #print(name, module._wrapped_object.weight.grad is not None)
                    ...
                    self.loss_dict[name] = self.criterion(output[is_labeled], target[is_labeled]).item()
